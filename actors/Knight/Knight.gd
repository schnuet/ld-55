...
func _enter_prepare():
	animated_sprite.play("attack_big");
...
func _update_chase(delta: float) -> void:
	...
	var distance_to_player = player_position - global_position;
		
	if (
		navigation_agent.is_navigation_finished() or
		navigation_agent.distance_to_target() < 7 or
		abs(distance_to_player.y) < 7 && abs(distance_to_player.x) < 40
	):
